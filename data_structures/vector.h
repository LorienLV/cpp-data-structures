...
public:
    ...
    public:
         ...
         * @return A new iterator with the added value.
         */
        [[nodiscard]] friend Iterator operator+(difference_type n,
         ...
         */
        [[nodiscard]] friend difference_type operator-(const Iterator &x,
    ...
    Vector &operator=(const Vector &other) {
        ...
        const bool reallocate =
            (propagate_alloc && alloc_ != other.alloc_) || capacity_ != other.capacity_;
     ...
     */
    friend void swap(Vector &x, Vector &y) noexcept(Vector::noexcept_swap) { x.swap(y); }
...
private:
    ...
    static constexpr bool avoid_init =
        AvoidInitIfPossible && std::is_standard_layout_v<T> && std::is_trivial_v<T>;
    ...
    void args_construct_element(T *pdst, Args &&...args) {
        alloc_traits::construct(
            alloc_, std::to_address(pdst), std::forward<Args>(args)...);
    }
